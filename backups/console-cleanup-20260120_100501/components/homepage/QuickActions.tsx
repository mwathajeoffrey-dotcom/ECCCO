...
import { FileText, Clock, Library, BarChart3, Users, BookOpen, Sparkles } from "lucide-react";
      ...
      <Link href={href}>
        ...
        >
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${gradient} rounded-3xl opacity-0 group-hover:opacity-20 blur-xl transition-opacity`}
            />
          ...
          {actions.map((action) => (
            <motion.div key={action.title} variants={item}>
              <TiltCard href={action.href} gradient={action.gradient}>
                <div
                  className={`w-16 h-16 ${action.iconBg} rounded-2xl flex items-center justify-center mb-6 shadow-2xl`}
                >
                  <action.icon className="w-8 h-8 text-white" />
                </div>

                <h3 className="text-3xl font-black text-white mb-2">{action.title}</h3>
                <p className="text-xl font-semibold text-gray-300 mb-1">{action.description}</p>
