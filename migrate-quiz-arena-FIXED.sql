-- Quiz Arena Tables Migration - FIXED VERSION
...
-- Create function if it doesn't exist (skip if already exists)
DO $$
BEGIN
...
DROP TRIGGER IF EXISTS update_quiz_session_updated_at ON "QuizSession";

CREATE TRIGGER update_quiz_session_updated_at
    BEFORE UPDATE ON "QuizSession"
    FOR EACH ROW
