...
const validateQuiz = (): string | null => {
  ...
  if (selectedQuestions.length === 0) return 'Please select at least one question';
...
**Medium Priority** (30-45 min each):
5. All other API routes - Add api-client usage
6. All live-quiz pages - Same fixes as quiz-arena
7. Consolidate Prisma imports to ONE file
8. Create `.env.example` documentation

**Low Priority** (when you have time):
9. Remove remaining console.logs (50+)
10. Add error boundaries to React components
11. Create custom hooks (useFetch, useAsync)
