...
export const track = {
  ...
  apiCall: <T>(endpoint: string, method: string, fn: () => Promise<T>) => 
    monitoring.trackApiCall(endpoint, method, fn),
  dbQuery: <T>(operation: string, model: string, fn: () => Promise<T>) => 
    monitoring.trackDbQuery(operation, model, fn),
  cache: (hit: boolean, key: string, namespace: string) => 
    monitoring.trackCacheEvent(hit, key, namespace),
