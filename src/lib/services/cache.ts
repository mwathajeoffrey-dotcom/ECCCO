 ...
 * ```
 */
export async function getCache<T>(key: string, namespace: string = "default"): Promise<T | null> {
  const cacheKey = namespace ? `${namespace}:${key}` : key;
  const client = getRedisClient();
 ...
 * @param namespace - Optional namespace
 */
export async function deleteCache(key: string, namespace: string = "default"): Promise<void> {
  const cacheKey = namespace ? `${namespace}:${key}` : key;
  const client = getRedisClient();
 ...
 * ```
 */
export async function getCached<T>(key: string, fetchFn: () => Promise<T>, options: CacheOptions = {}): Promise<T> {
...
export const cache = {
   ...
   */
  evidenceSearch: {
    get: (query: string) => getCache<any>(`search:${query}`, "evidence"),
   ...
   */
  quiz: {
    get: (id: string) => getCache<any>(`quiz:${id}`, "quizzes"),
   ...
   */
  user: {
    get: (id: string) => getCache<any>(`user:${id}`, "users"),
    ...
    invalidate: (id: string) => deleteCache(`user:${id}`, "users"),
  },
   ...
   */
  topics: {
    get: (category: string) => getCache<any>(`topics:${category}`, "content"),
