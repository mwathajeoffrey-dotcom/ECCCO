...
export function NoteModal({ isOpen, onClose, searchQuery, evidenceSummary, onSave, existingNote }: NoteModalProps) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      {isMinimized ? (
        <div className="fixed bottom-4 right-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-3 min-w-[300px]">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{title || searchQuery}</p>
        ...
        <div className={`flex min-h-full items-center justify-center p-4 ${isFullScreen ? "p-0" : ""}`}>
          <div
            className={`relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl transform transition-all ${
              isFullScreen ? "w-screen h-screen rounded-none" : "w-full max-w-3xl"
            }`}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Document your findings and insights</p>
                </div>
              </div>
            ...
            {/* Content */}
            <div
              className={`p-6 space-y-6 overflow-y-auto ${isFullScreen ? "max-h-[calc(100vh-200px)]" : "max-h-[70vh]"}`}
            >
              {/* Search Context */}
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-blue-900 dark:text-blue-100">Search Query</p>
                    <p className="text-sm text-blue-700 dark:text-blue-300 mt-1 break-words">{searchQuery}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Note Content */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Your Notes *</label>
                <RichTextEditor
                  value={content}
                  onChange={setContent}
